...
def provision_api():
    ...
    sudo('apt-get install --yes %s --no-upgrade' % ' '.join(PACKAGES))
...
def load_api_data():
    ...
    commands = (
        '%s -f %s/demarcation_2009.sql' % (PSQL_STRING, data_dir_abs),
        'for fp in `ls %s/*.tar.gz | xargs`; do tar -xvzf ${fp} -C %s/; done'
        % (data_dir_abs, data_dir_abs),
        'for fp in `ls %s/*.sql | grep -v demarcation_2009.sql | xargs`; '
