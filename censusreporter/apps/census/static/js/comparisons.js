...
function Comparison(options) {
    ...
    comparison.init = function(options) {
        ...
        // boundary data
        comparison.boundaryUrls = options.boundaryUrls;
    ...
    comparison.makeGridHeader = function() {
        ...
        comparison.sortedPlaces.forEach(function(g) {
            ...
            gridHeaderBits.push('<a href="/profiles/' + geoID + '-' + slugify(shortName) + '">' + geoName + '</a>');
        })
    ...
    comparison.loadMapData = function(cb) {
        // load all country and county boundary data
        var counter = _.size(comparison.boundaryUrls);
        var featureMap = {};

        _.each(comparison.boundaryUrls, function(url, level) {
            d3.json(url, function(error, topo) {
                if (error) return console.warn(error);

                var features = topojson.feature(topo, topo.objects[level]);
                ...
                if (--counter == 0) {
                    ...
                    _.each(comparison.dataGeoIDs, function(geoid) {
                        usefulFeatures[geoid] = featureMap[geoid];
                    });
